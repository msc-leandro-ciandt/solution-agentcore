...
class TestAuthentication:
    ...
    def test_unauthenticated_shows_sign_in_button(
        self, driver: webdriver.Chrome, BASE_URL: str = "https://main.d3de0r2ujefnqj.amplifyapp.com"
    ):
    ...
    def test_successful_login_redirects_to_chat(
        ...
        chat_interface = wait.until(
            EC.presence_of_element_located((By.TEXT_XPATH, "//*[contains(text(), 'Welcome to FAST Chat')]"))
        )

        assert chat_interface is not None, "Chat interface should be visible after login"
...
class TestChatInterface:
    ...
    def test_chat_input_visible(self, driver: webdriver.Chrome):
        ...
        input_field = wait.until(
            EC.presence_of_element_located((By.XPATH, "//textarea[@placeholder='Type your message...']"))
        )
    ...
    def test_can_send_message(
        ...
        assert test_message in input_field.get_attribute("value"), "Text should appear in input field"

        # Find and click the send button
        send_button = driver.find_element(By.XPATH, "//button[contains(@class, 'bg-blue') or contains(@class, 'primary')]")
        send_button.click()

        # Verify input is cleared
        wait.until(lambda d: input_field.get_attribute("value") == "")

        assert input_field.get_attribute("value") == "", "Input should be cleared after sending"

    def test_message_appears_in_chat(
        self, driver: webdriver.Chrome
    ):
        ...
        user_message = driver.find_element(By.XPATH, f"//*[contains(text(), '{test_message}')]")
        ...
        try:
            assistant_message = wait.until(
                EC.presence_of_element_located(
                    (By.XPATH, "//*[@class='assistant-message' or contains(@class, 'assistant')]")
                ),
...
class TestSessionSelection:
    ...
    def test_can_select_session_from_sidebar(
        self, driver: webdriver.Chrome
    ):
        ...
        session_items = wait.until(
            lambda d: d.find_elements(By.XPATH, "//*[contains(@class, 'session-item') or contains(@class, 'group')]"),
            timeout=5
        )
    ...
    def test_session_history_displays_correctly(
        self, driver: webdriver.Chrome
    ):
        ...
        if len(session_items) > 0:
            ...
            messages_area = wait.until(
                EC.presence_of_element_located(
                    (By.XPATH, "//*[@class='chat-messages' or contains(@class, 'messages')]")
                ),
                timeout=5
            )

            assert messages_area is not None, "Messages area should be visible after selecting session"
